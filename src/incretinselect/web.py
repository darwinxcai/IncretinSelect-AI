...
def _result_html(result: dict[str, Any]) -> str:
    ...
    <section class="panel">
      <h2>Does the model recognize this sequence?</h2>
      <p><span class="tier">{html.escape(app['tier'])}</span></p>
    ...
    <section class="panel">
      <h2>Limitations you should not skip</h2><ul>{warnings}</ul>
...
<body><main>
  <header class="hero"><div class="eyebrow">Local · frozen model · research only</div>
    ...
    <p>Estimate how an incretin-like peptide sequence may activate GLP-1R and GCGR in a
       cell-based cAMP assay. The purpose is early computational triage—not proof that a
       candidate binds, works, or is safe.</p>
  </header>
  ...
  <section class="panel"><form method="post" action="/predict">
    ...
    <button type="submit">Estimate functional potency</button>
  </form></section>
  {error_html}{detail}
  <section class="panel"><h2>What this app is for</h2>
    <p>It turns a compatible sequence into reproducible point estimates that can help form
      hypotheses about receptor balance. Development error was approximately 4.2-fold for
      GCGR, 11.7-fold for GLP-1R, and 13.7-fold for balance; these are population summaries,
      not per-sequence confidence intervals. The separate 15-peptide result was mixed.</p>
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.smoke_test:
        ...
        if "Research estimate" not in page or result["model"]["artifact_id"] not in page:
